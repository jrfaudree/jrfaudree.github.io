...
\item (5 points) Sketch a graph of the piecewise defined function:\\
 $f(x) =
  \begin{cases} 
      ...
      \hfill 1-x \hfill & \text{ if $2\leq x$} \\
  \end{cases}$
...
\begin{tikzpicture}[domain=-3:5, scale=.75]
    \draw[very thin,color=gray] (-3.1,-3.1) grid (4.9,4.9);
    \draw[->] (-3.2,0) -- (4.2,0) node[right] {$x$};
    \draw[->] (0,-3.2) -- (0,4.2) node[above] {$y$};
    \draw[ ultra thick] (-2,-1) -- (0,0);
    \draw[ultra thick] (0,0) parabola bend (1,2) (2,0);
    \draw[ultra thick] (2,0) -- (3,4);
    \node[circle,draw=black,fill=black, inner sep=2pt] at (-2,-1){};
    \node[circle,draw=black,fill=white, inner sep=2pt] at (3,4){};
    \node at (-1,1){$f(x)$};
    \node at (-2,-.3){$-2$};\node at (-3,-.3){$-3$};\node at (1,-.3){$1$};\node at (2,-.3){$2$};
    \node at (-.3,2){$2$};\node at (-.4,-2){$-2$};
    \end{tikzpicture}
