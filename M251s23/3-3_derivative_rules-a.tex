...
\end{center}
\begin{enumerate}
\item Review from Section 3.2.
\begin{enumerate}
	\item State the definition of $f'(x)$ using the ``$h$"-notation and use it to find $f'(x)$ for at least one of the functions in the list: $f_1(x)=C$, $f_2(x)=x$, $f_3(x)=x^2$, $f_4(x)=x^3$, $f_5(x)=x^{-1}$, $f_6(x)=x^{1/2}.$
	\vfill
...
\draw[<->,ultra thick,
      ...
      plot (\x,{cos(deg{\x})});
    \end{tikzpicture} 
 \item Use the work from the previous problems to fill in the blanks below:
 \begin{multicols}{2}
 \begin{enumerate}
 	\item $\frac{d}{dx}\left[ C\right]=$
	\item $\frac{d}{dx}\left[ x^n \right]=$
	\item $\frac{d}{dx}\left[\sin(x) \right]=$
